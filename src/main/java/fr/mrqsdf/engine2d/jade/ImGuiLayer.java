...
import fr.mrqsdf.engine2d.editor.*;
import fr.mrqsdf.engine2d.editor.input.InputNewFile;
...
public class ImGuiLayer {
    ...
    private PropertiesWindows propertiesWindows;
    ...
    private List<InputNewFile> inputNewFiles = new ArrayList<>();

    public ImGuiLayer(long glfwWindow, PickingTexture pickingTexture) {
        this.glfwWindow = glfwWindow;
        this.gameViewWindow = new GameViewWindow();
        this.propertiesWindows = new PropertiesWindows(pickingTexture);
        this.menuBar = new MenuBar();
        this.sceneHeirarchyWindow = new SceneHierarchyWindow();
        this.assetsWindow = new AssetsWindow();
        this.levelsWindow = new LevelsWindow();
        this.contentBrowserPanel = new ContentBrowserPanel();
        inputNewFiles.add(new SpriteSheetInput());
        inputNewFiles.add(new LevelInput());
    }
    ...
    public <T extends InputNewFile> T getInputNewFile(Class<T> clazz){
        for (InputNewFile inputNewFile : inputNewFiles){
            if (clazz.isAssignableFrom(inputNewFile.getClass())){
                try {
                    return clazz.cast(inputNewFile);
...
//        ImGui.showDemoWindow();
        if (Window.DEBUG_BUILD && Window.get().runtimePlaying) {
            propertiesWindows.imgui();
        } else if (!Window.get().runtimePlaying) {
            propertiesWindows.imgui();
        }
...
//
    ...
    public PropertiesWindows getPropertiesWindows(){
        return propertiesWindows;
    }
}
