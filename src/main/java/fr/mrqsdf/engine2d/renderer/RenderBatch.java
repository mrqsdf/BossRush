...
public class RenderBatch implements Comparable<RenderBatch> {
    ...
    public boolean spritePresent(SpriteRenderer spr){
        for (int i = 0; i < numSprites; i++){
            if (sprites[i].equals(spr)){
                return true;
            }
    ...
    private void loadVertexPropoerties(int index){
        ...
        if (isRotated){
            ...
            transformMatrix.rotate((float) Math.toRadians(sprite.gameObject.transform.rotation), new Vector3f(0,0,1));
        ...
        for (int i=0; i < 4; i++) {
            ...
            if (isRotated){
                currentPos = new Vector4f(xAdd, yAdd, 0.0f, 1.0f).mul(transformMatrix);
            }
